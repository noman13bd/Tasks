...
use Task\Validate;
use Task\Database\EntityInterface;
use Task\Database\EntityMapper;
...
class ObjectModel {
    ...
    public function add($auto_date = true, $null_values = false)
    {
        ...
        if ($auto_date && property_exists($this, 'updated_at')) {
    ...
    public function validateField($field, $value, $skip = array(), $human_errors = false)
    {
        ...
        if (!in_array('validate', $skip) && !empty($data['validate'])) {
            if (!method_exists('Task\Validate', $data['validate'])) {
                throw new \Exception(
            ...
            if (!empty($value)) {
                ...
                if (Tools::strtolower($data['validate']) == 'iscleanhtml') {
                    if (!call_user_func(array('Task\Validate', $data['validate']), $value, $hp_allow_html_iframe)) {
                        $res = false;
                    }
                } else {
                    if (!call_user_func(array('Task\Validate', $data['validate']), $value)) {
                        $res = false;
                    }
                }
                if (!$res) {
